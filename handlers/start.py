...
from app.keyboards.reply import main_kb, profile_setup_kb
...
async def start(message: Message) -> None:
    ...
    if message.from_user.id not in captcha_passed:
        if message.from_user.id not in captcha_waiting:
            ...
            await message.answer("Сначала завершите проверку и отправьте ответ на CAPTCHA.")
        return

    await message.answer("Добро пожаловать в анонимный чат.", reply_markup=main_kb())
    await message.answer(
        "Сначала рекомендую заполнить профиль и настройки поиска. "
        "Так матчи будут заметно лучше.",
        reply_markup=profile_setup_kb(),
    )
    await message.answer("Выберите язык поиска:", reply_markup=language_kb(user.language))
...
async def captcha_answer(message: Message) -> None:
    ...
    captcha_passed.add(message.from_user.id)
    await message.answer("✅ Проверка пройдена.", reply_markup=main_kb())
    await message.answer(
        "Профиль можно настроить в разделе «⚙️ Настройки», а потом сразу начать поиск.",
        reply_markup=profile_setup_kb(),
    )
